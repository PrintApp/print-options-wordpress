...
class Product_Options_Product_Tab
{
    ...
    {
        $tabs['print_options'] = [
            'label'    => __('Print Options', 'printapp-product-options'),
    ...
    public static function render_panel(): void
    {
        ...
        <div id="print_options_product_data" class="panel woocommerce_options_panel">
            <div class="options_group">
                <p class="form-field">
                    <label for="po_set_key"><?php esc_html_e('Print Options', 'printapp-product-options'); ?></label>
                    <select id="po_set_key" name="po_set_key" style="min-width: 260px;">
                        <option value=""><?php esc_html_e('None — sell at the normal price', 'printapp-product-options'); ?></option>
                        <?php foreach ($library as $set) : ?>
                            ...
                            $title = isset($set['title']) && is_string($set['title']) && '' !== $set['title']
                                ? $set['title']
                                : __('Untitled options', 'printapp-product-options');
                            ?>
                            <option
                ...
                <?php if (!$library) : ?>
                    <p>
                        ...
                        printf(
                            ...
                            esc_html__('You haven\'t created any print options yet — %s to build your first set.', 'printapp-product-options'),
                            '<a href="' . esc_url($manage) . '">' . esc_html__('open Print Options', 'printapp-product-options') . '</a>'
                        );
                        ?>
                    </p>
                <?php else : ?>
                    <p>
                        ...
                        printf(
                            /* translators: %s: link to the Print Options page */
                            esc_html__('Customers configure and price their job right on the product page. Manage the sets themselves on the %s page.', 'printapp-product-options'),
                            '<a href="' . esc_url($manage) . '">' . esc_html__('Print Options', 'printapp-product-options') . '</a>'
                        );
                        ?>
                    </p>
                <?php endif; ?>
        ...
        if ('' === $selected) {
            ...
            if (!$result['ok']) {
                self::notify(sprintf(
                    ...
                    __('Print Options could not be removed from this product: %s', 'printapp-product-options'),
                    $result['error']
                ));
                return;
            }
            ...
            if (!$result['ok']) {
                self::notify(sprintf(
                    /* translators: %s: error message */
                    __('Print Options could not be assigned to this product: %s', 'printapp-product-options'),
                    $result['error']
                ));
                return;
            }
