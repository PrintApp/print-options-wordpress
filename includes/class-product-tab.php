...
class PAPO_Product_Tab
{
    ...
    {
        $tabs['print_options'] = [
            'label'    => __('Print Options', 'print-app-product-options-for-woocommerce'),
                    ...
                    <select id="papo_set_key" name="papo_set_key" style="min-width: 260px;">
                        ...
                        <?php foreach ($library as $set) : ?>
                            ...
                            $title = isset($set['title']) && is_string($set['title']) && '' !== $set['title']
                                ? $set['title']
                                : __('Untitled options', 'print-app-product-options-for-woocommerce');
                            ?>
                            <option
                ...
                <?php if (!$library) : ?>
                    <p>
                        ...
                        printf(
                            ...
                            esc_html__('You haven\'t created any print options yet — %s to build your first set.', 'print-app-product-options-for-woocommerce'),
                            '<a href="' . esc_url($manage) . '">' . esc_html__('open Print Options', 'print-app-product-options-for-woocommerce') . '</a>'
                        );
                        ?>
                    </p>
                <?php else : ?>
                    <p>
                        ...
                        printf(
                            /* translators: %s: link to the Print Options page */
                            esc_html__('Customers configure and price their job right on the product page. Manage the sets themselves on the %s page.', 'print-app-product-options-for-woocommerce'),
                            '<a href="' . esc_url($manage) . '">' . esc_html__('Print Options', 'print-app-product-options-for-woocommerce') . '</a>'
                        );
                        ?>
                    </p>
                <?php endif; ?>
        ...
        if ('' === $selected) {
            ...
            if (!$result['ok']) {
                self::notify(sprintf(
                    ...
                    __('Print Options could not be removed from this product: %s', 'print-app-product-options-for-woocommerce'),
                    $result['error']
                ));
                return;
            }
            ...
            if (!$result['ok']) {
                self::notify(sprintf(
                    /* translators: %s: error message */
                    __('Print Options could not be assigned to this product: %s', 'print-app-product-options-for-woocommerce'),
                    $result['error']
                ));
                return;
            }
