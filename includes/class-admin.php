...
class Product_Options_Admin
{
    ...
    public static function menu(): void
    {
        ...
        add_menu_page(
            __('Print Options', 'print-options'),
            __('Print Options', 'print-options'),
            'manage_woocommerce',
            self::PAGE_SLUG,
            [self::class, 'render'],
    ...
    public static function render(): void
    {
        ...
        <div class="wrap" style="height: calc(100vh - 65px); display: flex; flex-direction: column;">
            <iframe
                ...
                title="<?php esc_attr_e('Print Options builder', 'print-options'); ?>"
    ...
    public static function handle_bridge(): void
    {
        ...
        if (!current_user_can('manage_woocommerce')) {
            wp_send_json_error(['message' => __('You are not allowed to manage Print Options.', 'print-options')], 403);
        }

        $method = isset($_POST['method']) ? sanitize_key(wp_unslash($_POST['method'])) : '';
        if (!isset(self::METHODS[$method])) {
            wp_send_json_error(['message' => __('Unknown request.', 'print-options')], 400);
        }
    ...
    private static function handle_library_save(array $params): void
    {
        ...
        if ('' === $set_key || null === $config) {
            wp_send_json_error(['message' => __('Invalid option set.', 'print-options')], 400);
        }
